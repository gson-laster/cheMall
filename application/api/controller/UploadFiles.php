...
class UploadFiles extends Controller
{
  ...
  public function uploadAjax($file, $to_dir, $ajaxSuccCode = 1, $ajaxErrCode = 0, $compress = false){
    ...
    if($info){
        // 压缩
        if(boolval($compress)){
          $img = (new Imgcompress(WEB_ROOT.'/public/upload'. DS .$to_dir.DS.$info->getSaveName(), 1))->compressImg(WEB_ROOT.'/public/upload'. DS .$to_dir.DS.$info->getSaveName());
        }
  ...
  function saveBase64Image($to_dir, $compress = false){
      ...
      if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)){
          ...
          $image_url = Config::get('upload_main_path'). DS .$to_dir. DS.date('Y/m-d').DS .$image_name;
          //dump($image_url);
          ...
          if (file_put_contents($image_url, $decode)){
            // 压缩
            if(boolval($compress)){
              $img = (new Imgcompress(WEB_ROOT.'/public/upload'. DS .$to_dir. DS.date('Y/m-d').DS .$image_name, 1))->compressImg(WEB_ROOT.'/public/upload'. DS .$to_dir. DS.date('Y/m-d').DS .$image_name);
            }
                $data['code']=0;
                $data['imageName']=$image_name;
                $data['url']='/public/upload/'.$to_dir.'/'.date('Y/m-d').'/'.$image_name;
                $data['msg']='保存成功！';
          }else{
            $data['code']=1;
            $data['imgageName']='';
