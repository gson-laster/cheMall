...
class Recharge extends Controller {
  ...
  public function addSuccess($apply_recharge, $userId){
    $this->buildActive($apply_recharge, $userId, 'Recharge@addSuccess');
  }
  ...
  public function confirm($id, $res){
    $this->buildActive($id, $res['user_id'], 'Recharge@confirm');
  }
  /**
   * 退回
   * @author: slide
  ...
  public function returnToUser($res){
    $this->buildActive($res['id'], $res['user_id'],'Recharge@returnToUser');
  }
  ...
  public function OpenAgentFirst($orderId, $userId){
    $flag = false;
    $apply_res = Db::name('apply_recharge')->field('id')->where("user_id", $userId)->select();
    $apply_res_ids = removeArrValue($apply_res, 'id', $orderId);
    $apply_res_key = array_keys(convert_arr_key($apply_res_ids, 'id'));
    if(count($apply_res_key) > 0){
      $flag = false;
    }else{
      $flag = true;
    }
